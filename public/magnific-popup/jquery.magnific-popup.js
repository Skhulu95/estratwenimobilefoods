				...
				if(el.length > 0) {
					...
					if(attr === 'replaceWith') {
						...
						if(el.is('img')) {
							...
							el.replaceWith( $('<img>').attr('src', value).attr('className', el.attr('className')) );
						}
					} else {
						el.attr(arr[1], value);
					}
...
var AJAX_NS = 'ajax',
	...
	_removeAjaxCursor = function() {
		if(_ajaxCur) {
			$(document.body).removeclassName(_ajaxCur);
		}
	},
...
$.magnificPopup.registerModule(AJAX_NS, {
	...
	proto: {
		...
		getAjax: function(item) {
			...
			if(_ajaxCur) {
				$(document.body).addclassName(_ajaxCur);
			}
		...
		initImage: function() {
			...
			_mfpOn(OPEN_EVENT+ns, function() {
				if(mfp.currItem.type === 'image' && imgSt.cursor) {
					$(document.body).addclassName(imgSt.cursor);
				}
			});

			_mfpOn(CLOSE_EVENT+ns, function() {
				if(imgSt.cursor) {
					$(document.body).removeclassName(imgSt.cursor);
				}
